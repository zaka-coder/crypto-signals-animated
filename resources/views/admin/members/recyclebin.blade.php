  ...
  <script>
    ...
    function restoreMember(memberId) {
      if (confirm('Are you sure you want to restore this member?')) {
        ...
        $.ajax({
          url: `/customers/${memberId}/restore`,
          type: 'POST',
          success: function(response) {
            if (response.success) {
              ...
              let table = $('#deleted-members').DataTable();
              let row = table.row($(`#deleted-members tbody tr[data-id="${memberId}"]`));
              row.remove().draw(); // Removes the row and redraws the table

            } else {
              alert('Error: Unable to restore member.');
            }
          },
          error: function() {
